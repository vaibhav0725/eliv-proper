...
const DEFAULT_TRANSITION = { duration: 0.45 };
...
const Text3DFlip = ({
  ...
  const handleFlip = useCallback(async () => {
    ...
    try {
      ...
      const base = CONTAINER_TRANSFORMS[rotateDirection];
      const chars = scope.current.querySelectorAll<HTMLElement>(
        ".text-3d-flip-char",
      );

      await Promise.all(
        Array.from(chars).map((element, index) =>
          element
            .animate(
              [
                { transform: base },
                { transform: `${base} ${rotationTransform}` },
              ],
              {
  ...
  }, [characters, transition, getStaggerDelay, rotationTransform, rotateDirection]);

  return (
    <ElementTag
      className={cn("relative flex flex-wrap perspective-[400px]", className)}
    ...
    >
      ...
      {characters.map((wordObj, wordIndex) => (
        <span key={wordIndex} className="inline-flex" aria-hidden>
